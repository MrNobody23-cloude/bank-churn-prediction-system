...
const fullDataset = [...baseCustomers];

for (let i = baseCustomers.length; i < 800; i++) {
  const seed = i * 12345;
  const age = Math.floor(seededRandom(seed) * 60) + 18;
  const geography = geographies[Math.floor(seededRandom(seed + 1) * 3)];
  const numProducts = Math.floor(seededRandom(seed + 2) * 3) + 1;
  const isActive = seededRandom(seed + 3) > 0.5 ? 1 : 0;
  
  let churnBase = 0.15;
  if (age > 50) churnBase += 0.15;
  if (geography === "Germany") churnBase += 0.1;
  if (numProducts > 2) churnBase += 0.3;
  if (isActive === 0) churnBase += 0.1;
  
  fullDataset.push({
    Year: 2025,
    CustomerId: 15600000 + i,
    Surname: surnames[Math.floor(seededRandom(seed + 4) * surnames.length)],
    CreditScore: Math.floor(seededRandom(seed + 5) * 500) + 350,
    Geography: geography,
    Gender: genders[Math.floor(seededRandom(seed + 6) * 2)],
    Age: age,
    Tenure: Math.floor(seededRandom(seed + 7) * 11),
    Balance: seededRandom(seed + 8) > 0.3 ? Math.round(seededRandom(seed + 9) * 200000 * 100) / 100 : 0,
    NumOfProducts: numProducts,
    HasCrCard: seededRandom(seed + 10) > 0.3 ? 1 : 0,
    IsActiveMember: isActive,
    EstimatedSalary: Math.round((seededRandom(seed + 11) * 150000 + 30000) * 100) / 100,
    Exited: seededRandom(seed + 12) < churnBase ? 1 : 0
  });
}
